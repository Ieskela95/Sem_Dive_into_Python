import json
from home_work.home_work_15.loc_dir.log import logger
...
class ModelCashMachine:
    ...
    def __division_check_50(self, value):
        ...
        logger.debug('Проверка деления на 50')
    ...
    def put_money(self, add):
        ...
        logger.error(f'Ошибка внесения: неверная сумма {add}!')
        return f'Ошибка внесения: неверная сумма {add}!'

    def give_money(self, take):
        """Метод для снятия денег"""
        logger.debug('Метод для снятия денег')
        if self.__division_check_50(take):
            ...
            if self.__cash < (take + percent):
                ...
                logger.error(f'Ошибка снятия денег: запрос {take} у.е, недостаточно средств!')
                ...
                logger.info(f'запрос {take} у.е, успешно (комиссия за трансакцию: {percent} у.е)!')
    ...
    def give_percent(self):
        ...
        if self.__counter % self.__BONUS_OPERATION == 0:
            ...
            logger.info(f'Начисления за {self.__counter} операцию ! Банк начислил проценты по вкладу !')
    ...
    def save_json(self):
        ...
        logger.debug('Метод сохранения состояния в JSON')
        with open(self.__path_save, 'w', encoding='utf-8') as file:
            ...
            logger.info('Файл save.json сохранен в данной директории!')
            return 'Файл save.json сохранен в данной директории!'

    def load_json(self):
        """Метод загрузки состояния из JSON """
        logger.debug('Метод загрузки состояния из JSON ')
        with open(self.__path_save, 'r', encoding='utf-8') as file:
            ...
            logger.info('Файл save.json загружен из данной директории...баланс, история операций и счетчик обновлены !')
